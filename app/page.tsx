          ...
          <div className="max-w-2xl">
            ...
            <div className="mt-16 grid gap-1 sm:grid-cols-3 bg-white/10 p-[1px]">
              <div className="bg-neutral-950 p-6 transition-colors hover:bg-neutral-900">
                ...
                <p className="mt-3 text-sm font-medium text-white uppercase tracking-wide">
                  Architectural Vinyl
                </p>
              </div>
              <div className="bg-neutral-950 p-6 transition-colors hover:bg-neutral-900">
            ...
            <div className="grid gap-6 sm:grid-cols-2">
              {products.slice(0, 4).map((product, index) => (
                <Link
                  ...
                  className={`group relative overflow-hidden bg-neutral-900 border border-white/5 transition-all duration-500 hover:-translate-y-2 hover:border-[#D4AF37]/50 hover:shadow-[0_20px_40px_rgba(212,175,55,0.1)] ${
                    index % 2 === 1 ? "sm:mt-12" : ""
                  }`}
                >
                  <div className="h-48 overflow-hidden bg-white/5 p-4 flex items-center justify-center">
                    <img
                      src={product.image}
                      alt={product.title}
                      className="max-h-full max-w-full object-contain transition duration-700 group-hover:scale-110 drop-shadow-xl"
                    />
                  </div>
                  <div className="border-t border-white/5 p-5">
                    <p className="line-clamp-1 text-sm font-bold uppercase tracking-wide text-white group-hover:text-[#D4AF37] transition-colors">
                      {product.title}
                    </p>
                    <p className="mt-2 text-sm font-black text-[#BC2229]">
      ...
      <section className="bg-white px-6 py-24 md:py-32 lg:px-8">
        <div className="mx-auto max-w-7xl">
          <div className="flex flex-col gap-6 md:flex-row md:items-end md:justify-between border-b-2 border-black pb-8">
            ...
            <Link
              href="/shop"
              className="inline-flex w-fit items-center border border-black bg-white px-8 py-3 text-xs font-bold uppercase tracking-[0.2em] text-black transition-all hover:bg-black hover:text-[#D4AF37]"
            >
              View Full Index
            </Link>
          </div>
          ...
          <div className="mt-16 grid gap-8 sm:grid-cols-2 xl:grid-cols-4">
            {products.map((product) => (
              <article
                key={product.id}
                className="group relative flex flex-col border border-neutral-200 bg-white transition-all duration-300 hover:-translate-y-1 hover:border-black hover:shadow-[8px_8px_0px_0px_#BC2229]"
              >
                ...
                <div className="flex flex-1 flex-col p-6">
                  <div className="mb-4 flex items-center justify-between gap-3">
                    <span className="bg-black px-3 py-1.5 text-[10px] font-bold uppercase tracking-[0.2em] text-[#D4AF37]">
                      {product.category}
                    </span>
                    <span className="text-[10px] font-bold uppercase tracking-widest text-neutral-400">
                      {product.quantity > 0 ? `QTY: ${product.quantity}` : "UNAVAILABLE"}
                    </span>
                  </div>

                  <h3 className="line-clamp-2 text-lg font-black uppercase leading-tight tracking-tight text-black">
                    {product.title}
                  </h3>

                  <div className="mt-auto pt-8 flex items-center justify-between">
                    <span className="text-2xl font-black text-[#BC2229]">
                      £{product.price.toFixed(2)}
                    </span>
                  </div>
                </div>
                ...
                <Link
                  href={`/products/${product.slug}`}
                  className="w-full bg-neutral-100 py-4 text-center text-xs font-bold uppercase tracking-[0.2em] text-black transition-all group-hover:bg-black group-hover:text-white"
                >
                  Inspect Item
                </Link>
              </article>
